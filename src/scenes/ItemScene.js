class ItemScene extends Phaser.Scene {
  ...
  createMobile() {
    this.cameras.main.setBackgroundColor('#8bd7ff');
    this.add.rectangle(360, 640, 720, 1280, 0xbcecff, 0.6);
    this.add.ellipse(360, 1260, 920, 220, 0x6ec655, 1);
    this.add.rectangle(360, 640, 666, 1200, 0xf8fbff, 0.96).setStrokeStyle(4, 0x2454a6);
    this.add.text(44, 54, this.bossReward ? 'Boss besiegt' : 'Wave geschafft', { fontSize: '38px', fontStyle: 'bold', color: '#2454a6' });
    this.add.text(46, 104, this.bossReward ? 'Vollheilung + Item' : 'Waehle ein Item', { fontSize: '24px', fontStyle: 'bold', color: '#2454a6' });
    this.add.text(46, 146, this.player.name + ' HP ' + Math.ceil(this.player.currentHp) + '/' + this.player.stats.hp + ' | Score ' + (this.runStats.score || 0) + ' | $ ' + (this.runStats.money || 0), {
      fontSize: '19px',
      color: '#2454a6',
    ...
    this.items = Phaser.Utils.Array.Shuffle(ITEMS.slice()).slice(0, 3);
    this.items.forEach(function (item, index) {
      this.drawItemCard(item, 360, 340 + index * 286);
    }, this);
  }

  drawItemCard(item, x, y) {
    if (this.isMobile) return this.drawMobileItemCard(item, x, y);
  ...
  drawMobileItemCard(item, x, y) {
    var rarityColor = item.category === 'special' ? 0xc77dff : item.category === 'stat' ? 0xffd166 : 0x7bd88f;
    var rect = this.add.rectangle(x, y, 604, 236, 0xffffff).setStrokeStyle(4, 0x2454a6).setInteractive({ useHandCursor: true });
    this.add.rectangle(x, y - 92, 604, 48, rarityColor).setStrokeStyle(2, 0x2454a6);
    this.add.text(x, y - 92, item.name, { fontSize: '26px', fontStyle: 'bold', color: '#111827', align: 'center', wordWrap: { width: 560, useAdvancedWrap: true } }).setOrigin(0.5);
    this.add.text(x, y - 34, item.category.toUpperCase(), { fontSize: '16px', fontStyle: 'bold', color: '#2454a6' }).setOrigin(0.5);
    this.add.text(x, y + 18, item.description, { fontSize: '21px', color: '#111827', align: 'center', wordWrap: { width: 540, useAdvancedWrap: true } }).setOrigin(0.5);
    this.add.text(x, y + 84, 'NEHMEN', { fontSize: '19px', fontStyle: 'bold', color: '#2454a6' }).setOrigin(0.5);
    rect.on('pointerdown', function () {
      BattleSystem.applyItem(this.player, item);
      var enemy = this.waveManager.nextWave(this.player.id);
      this.scene.start('BattleScene', {
